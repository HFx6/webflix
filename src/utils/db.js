...
db.on("populate", () => {
    db.users.bulkPut([{ username: "Bouba" }, { username: "Kiki" }]);
});
...
export async function updateLikedMedia(mediaId) {
    ...
    const isIn = await db.liked.where('[mediaId+userId]').equals([mediaId, userId]).first();
    if (isIn) {
        await db.liked.where('[mediaId+userId]').equals([mediaId, userId]).delete();
        return false;
    } else {
...
export async function updateWatchlistMedia(mediaId) {
    const currentUser = await getCurrentUser();
    const userId = currentUser[0]?.userId;
    const isIn = await db.watchlist.where('[mediaId+userId]').equals([mediaId, userId]).first();
    if (isIn) {
        await db.watchlist.where('[mediaId+userId]').equals([mediaId, userId]).delete();
        return false;
    } else {
...
export async function updateWatchedMedia(mediaId) {
    const currentUser = await getCurrentUser();
    const userId = currentUser[0]?.userId;
    const isIn = await db.watched.where('[mediaId+userId]').equals([mediaId, userId]).first();
    if (isIn) {
        await db.watched.where('[mediaId+userId]').equals([mediaId, userId]).delete();
        return false;
    } else {
